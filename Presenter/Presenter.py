        ...
        while flag:
            ...
            try:
                ...
                if choice == 1:
                    ...
                    Viewer.print_in_console(self.viewer, message=Message.saved_message)
                        ...
                        # If there is the only result, the app propose to make next choice: correct, delete or return
                            ...
                            if edit_choice < 1 or edit_choice > 3:
                                ...
                                if edit_choice == 1:
                                    ...
                                    Viewer.print_in_console(self.viewer, message=Message.correction_message)
                                if edit_choice == 2:
                                    FileManager.delete_note(search_result[0])
                                    Viewer.print_in_console(self.viewer, message=Message.deleted_message)
                                if edit_choice == 3:
                                    continue
                # User have chosen to display notes
                elif choice == 3:
                    # The app proposes to user to display all notes, filter notes by date or return
                    filter_choice = int(Viewer.get_data(self.viewer, message=Message.filter_choice_message))
                    if filter_choice < 1 or filter_choice > 3:
                        ...
                        # User has chosen to filter notes by date
                            ...
                            start_date = Viewer.get_data(self.viewer, message=Message.start_date)
                            end_date = Viewer.get_data(self.viewer, message=Message.end_date)
                            info = FileManager.filtered_notes(self.file_manager, start_date, end_date)
                            Viewer.print_list_in_console(self.viewer, notes=info)
                        # User has chosen to find a note by text fragment
                ...
                Viewer.print_in_console(self.viewer, message=Message.wrong_date)
                continue
